...
class MpRepo {
    ...
    protected function read(): void {
        $this->data = Json::read(Config::mpJsonFile());
    }
    ...
    protected function validate(): void {
        foreach ($this->data as $index => $item){
            ...
            if(key_exists('autopagar', $item) === false){
                throw new FailException("Campo [autopagar] não existe no índice [$index].");
            }
            ...
            if(is_bool($item['autopagar']) === false){
                throw new FailException("Campo [autopagar] não tem valor booleano no índice [$index].");
            }
        }
    }
    ...
    public function write(): void {
        $this->validate();
        Json::write($this->data, Config::mpJsonFile());
        $this->read();
    }

    public function add(string $nome, string $descricao, bool $ativo, bool $autopagar): int {
        if ($this->exist($nome)) {
    ...
    public function changeAutopagar(int $index, bool $autopagar): void {
        $this->indexExist($index);
        $this->data[$index]['autopagar'] = $autopagar;
        $this->write();
    }
