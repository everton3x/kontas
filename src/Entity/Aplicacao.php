...
class Aplicacao extends EntityBase {
    ...
    protected function exists(string $nome): bool {
        $stmt = $this->dbh->prepare('SELECT * FROM aplicacoes WHERE nome LIKE :nome');
    ...
    public function add(string $nome): int|bool {
        ...
        //calcula nova id
        $this->id = $this->nextIdFor('aplicacoes');

        //adiciona

        $this->dbh->beginTransaction();
        $stmt = $this->dbh->prepare('INSERT INTO aplicacoes (id, nome, ativo) VALUES (:id, :nome, 1)');
    ...
    public function load(int $id): bool {
        $stmt = $this->dbh->prepare('SELECT * FROM aplicacoes WHERE id LIKE :id');
    ...
    public function nome(): string {
        $stmt = $this->dbh->prepare('SELECT * FROM aplicacoes WHERE id = :id');
        $stmt->bindValue(':id', $this->id);
        if ($stmt->execute() === false) {
            throw new \PDOException("Falha ao executar consulta: {$stmt->errorInfo()}");
        }
    ...
    public function update(string|null $nome = null, bool|null $ativo): bool {
        ...
        if ($nome !== null && $ativo !== null) {
            $stmt = $this->dbh->prepare('UPDATE aplicacoes SET nome = :nome, ativo = :ativo WHERE id = :id');
            $stmt->bindValue(':id', $this->id);
            $stmt->bindValue(':nome', $nome);
            $stmt->bindValue(':ativo', $ativo, \PDO::PARAM_INT);
        } elseif ($nome !== null && $ativo === null) {
            $stmt = $this->dbh->prepare('UPDATE aplicacoes SET nome = :nome WHERE id = :id');
            $stmt->bindValue(':id', $this->id);
            $stmt->bindValue(':nome', $nome);
        } elseif ($nome === null && $ativo !== null) {
            $stmt = $this->dbh->prepare('UPDATE aplicacoes SET ativo = :ativo WHERE id = :id');
            $stmt->bindValue(':id', $this->id);
            $stmt->bindValue(':ativo', $ativo, \PDO::PARAM_INT);
        } else {
            return false;
    ...
    public function list(\PDOStatement|null $stmt = null): array {
        if ($stmt === null) {
            $stmt = $this->dbh->prepare('SELECT * FROM aplicacoes ORDER BY ativo DESC, nome ASC');
        }
        if ($stmt->execute() === false) {
            throw new \PDOException("Falha ao executar consulta: {$stmt->errorInfo()}");
        }
