    ...
    public function execute(): bool {
        ...
        switch (key($choice->answer())){
            case 't':
                ...
                break;
            case 'a':
                $stmt = $this->program->dbh()->prepare('SELECT * FROM aplicacoes WHERE ativo = 1 ORDER BY nome ASC');
                break;
            case 'i':
                $stmt = $this->program->dbh()->prepare('SELECT * FROM aplicacoes WHERE ativo = 0 ORDER BY nome ASC');
                break;
        }
